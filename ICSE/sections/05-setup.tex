...
\begin{itemize}
  ...
  agent retrieve effectively end to end, and does the \emph{same} system hold up
  as the available evidence degrades from full telemetry to text alone?
  \item[\textbf{RQ3}] \textbf{(Cost of autonomy).} What does the agent's
...
We compare our hybrid retriever and full agent against three groups of baselines,
...
runs on a single GPU per job; full configurations and seeds are released with the
artifact.
