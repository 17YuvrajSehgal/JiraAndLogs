...
curves, and expected calibration error. We also report pages per incident, which captures whether duplicate alerts are
suppressed. Finally,
...
\noindent\emph{Gold validation.} Because the real dataset's matches are inferred rather than human-labelled, we
validate them with an LLM-as-judge: a language model rates the relevance of each
gold match and of a random control, and we report the gap as evidence that the
labels carry signal.
