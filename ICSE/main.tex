...
We present \textbf{REMEDY}, an adaptive incident-triage framework that retrieves past incidents by fusing dense, lexical, and knowledge-graph retrieval and invokes additional reasoning only when uncertainty justifies its cost, letting one system span regimes from rich telemetry to text-only reports. We evaluate it on three datasets of increasing realism: two new telemetry-to-ticket datasets that pair multi-signal telemetry with incident tickets and per-window labels, and a real-world corpus of 38,642 resolved Apache Jira incidents across 24 projects. On real incidents, REMEDY retrieves the correct prior incident in its top five 97\% of the time, significantly outperforming strong dense, lexical, graph, and retrieval-augmented LLM baselines, while cutting triage cost by up to 78\% with no loss in quality.
A controlled synthetic-versus-real comparison further shows that the ticket-worthiness classification dominating existing benchmarks is nearly saturated on synthetic telemetry yet no better than chance on real incidents: the transferable challenge is retrieving the right past incident, not classifying alerts, reframing how triage systems should be evaluated. We release our datasets, code, evaluation harness, and results.
