...
Table~\ref{tab:cost-vs-cascade} reports cost in two configurations. The \emph{verifier-off} configuration is the lightweight default used when the language-model service is unavailable (the verifier is not registered). The \emph{verifier-on} configuration is the production-realistic one in which the always-on baseline does include the language-model verifier; this is the configuration the paper recommends as the headline cost result, because the verifier is the configuration that meaningfully matters in deployment.

\begin{table}[ht]
  \centering
  \caption{Cost on \dsob{}: agent versus the always-on cascade counterfactual on the same $1{,}008$ windows. The verifier-on row is the production-realistic configuration.}
  \label{tab:cost-vs-cascade}
  \footnotesize
  \setlength{\tabcolsep}{3pt}
  \begin{tabular}{@{}l rr rr@{}}
    \toprule
    & \multicolumn{2}{c}{Verifier-off} & \multicolumn{2}{c}{\textbf{Verifier-on}} \\
    \cmidrule(lr){2-3} \cmidrule(lr){4-5}
    Resource         & Agent  & Cascade & Agent     & Cascade   \\
    \midrule
    Wall time (s)    & $61.2$  & $101.9$  & $1{,}524$   & $4{,}783$   \\
    LLM tokens       & $71.6$k & $121.6$k & $258.7$k    & $720.2$k    \\
    USD-equivalent   & $\$0.018$ & $\$0.030$ & $\$0.065$ & $\$0.182$ \\
    \midrule
    \textbf{Savings} & \multicolumn{2}{c}{wall $40\%$ / USD $40\%$} & \multicolumn{2}{c}{\textbf{wall $68.1\%$ / USD $64.1\%$}} \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[ht]
  ...
  \begin{tabular}{@{}l rrr@{}}
    ...
    Cheap-path primitives$^\dagger$ & $1008$ & $0$   & $0.0\%$  \\
    \bottomrule
  \end{tabular}
  \\[2pt]
  {\scriptsize $^\dagger$\,Dense retriever, numeric-feature classifier, and the three composition primitives---run on every window by construction.}
\end{table}
...
\begin{table}[ht]
  \centering
  \caption{Tool-subset paired-delta CIs on \dsob{} (vs.\ the empty no-tools baseline). The single statistically-significant finding is a \emph{negative} one: the extended-trace tool harms Hit@1.}
  ...
  \begin{tabular}{@{}l r c r@{}}
    ...
    \midrule
    \textbf{Trace alone}            & $\mathbf{-0.0181}$ & $\mathbf{[-0.033, -0.006]}$ & $\mathbf{0.002}$ \\
    Trace $+$ metrics               & $-0.0181$ & $[-0.033, -0.006]$ & $0.002$ \\
    Events $+$ trace                & $-0.0121$ & $[-0.027,\;0.000]$ & $0.104$ \\
    Peers alone (best)              & $+0.0151$ & $[-0.006, +0.036]$ & $0.180$ \\
    All four tools (full ReAct)     & $+0.0121$ & $[-0.006, +0.030]$ & $0.262$ \\
    Events alone                    & $+0.0030$ & $[-0.009, +0.018]$ & $0.832$ \\
...
\begin{table}[ht]
  ...
  \begin{tabular}{@{}l r rr r r@{}}
    \toprule
    Dataset & $n_{\text{eval}}$ & BM25 H@1 & BM25 H@5 & Agent H@5 & Lift \\
    \midrule
    \dsob{}    & $331$ & $0.0514$ & $0.0876$ & $0.7583$ & $\mathbf{8.66\times}$ \\
    \dsotel{}  & $119$ & $0.3866$ & $0.5630$ & $0.7563$ & $1.34\times$ \\
    \dswol{}   & $48^\dagger$ & $0.004$ & $0.609$ & $1.000$ & $1.64\times$ \\
    \bottomrule
  \end{tabular}
  \\[2pt]
  {\scriptsize $^\dagger$\,Agent-evaluable subset; the always-on cascade baseline is evaluable on a wider $n=276$ subset, on which BM25 reports the same H@1/H@5 to two decimals.}
\end{table}

On the synthetic dataset BM25 collapses to Hit@5 below $9\%$ because the humanised Jira tickets use vocabulary that does not match the telemetry-side evidence text; the agent's $0.7583$ is $8.66\times$ better. On the real-Apache-Jira dataset BM25 retrieves the gold ticket in the top-five $60.9\%$ of the time but almost \emph{never} as the top-one ($\text{Hit@1} = 0.004$), because keyword overlap alone is insufficient to disambiguate among same-project tickets; the agent's $0.958$ Hit@1 (a $245\times$ relative lift over BM25 at the rank-one cut) is the difference between a system that always asks the engineer to pick and one that puts the right answer first $96\%$ of the time. The cross-dataset replication is the subject of \S\ref{sec:cross-app}.
