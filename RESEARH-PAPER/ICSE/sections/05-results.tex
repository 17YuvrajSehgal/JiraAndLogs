...
\begin{table}[ht]
  \centering
  \caption{Naive-baseline comparison: BM25-only retrieval versus the agent, on the agent-evaluable subset of each test split. \dswol{} reports two evaluable counts: the smaller one applies to the agent (the cheap-path gate closes confidently on most windows, so few escalations fire), the larger one applies to the always-on cascade baseline.}
  \label{tab:bm25}
  \small
  \begin{tabular}{lrrrrr}
    ...
    \dswol{}   & $48$ (agent) / $276$ (cascade) & $0.004$ & $0.609$ & $1.000$ & $1.64\times$ \\
    \bottomrule
  \end{tabular}
\end{table}
