...
# TCH-Lite (2026-06-11): log-only deployment configuration. Documented in
# docs7/TCH-Lite.md. When enabled, the cascade drops `hist_gradient_boosting_
# numeric` from the L1 stacker because the numeric-feature triage gate has
# no input on log-only datasets such as WoL. The L2 retrieval fusion, L3
# novelty disjunction, and L4 composition are unchanged structurally; the
# learned L3 classifier is expected to be a different artifact, refit on
# the log-only triage-score distribution (set TCH_LEARNED_NOVELTY_PATH to
# point at the refit file). The cascade's runtime cost is unchanged.
#
# Activation:
#     TCH_LITE=1
#         Drop HGB from L4_STACK_FEATURES.
#     TCH_LITE_OUTPUT_SUFFIX=lite (optional)
#         Suffix appended to the cascade's report.md filename so combined
#         + lite runs do not overwrite each other when they share an
#         --output-dir.
import os as _os_init

_TCH_LITE = _os_init.environ.get("TCH_LITE", "").strip() == "1"
if _TCH_LITE and "hist_gradient_boosting_numeric" in L4_STACK_FEATURES:
    L4_STACK_FEATURES.remove("hist_gradient_boosting_numeric")
    print("[build_cascade] TCH_LITE=1 — log-only mode: HGB dropped from L1 stacker; "
          f"L4_STACK_FEATURES now has {len(L4_STACK_FEATURES)} features.")
...
def main() -> None:
    ...
    suffix = _os_init.environ.get("TCH_LITE_OUTPUT_SUFFIX", "").strip()
    suffix_str = f"-{suffix}" if suffix else ""
    report_path = args.output_dir / f"report{suffix_str}.md"
    mode_name = "TCH-Lite (log-only)" if _TCH_LITE else "TCH-Combined"
    with report_path.open("w", encoding="utf-8") as fh:
        fh.write(f"# TCH Cascade Report — {mode_name}\n\n")
        ...
        if _TCH_LITE:
            fh.write(f"- TCH_LITE=1 active — HGB excluded from L1 stacker "
                     f"(see docs7/TCH-Lite.md).\n")
